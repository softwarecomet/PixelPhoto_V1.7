...
namespace PixelPhoto.Activities.Funding
{
    ...
    public class EditFundingActivity : AppCompatActivity
    {
        ...
        private async void TxtAddOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    var (apiStatus, respond) = await RequestsAsync.User.EditFunding(DataObject.Id.ToString(),TxtName.Text, TxtDescription.Text, TxtAmount.Text, PathImage);
                    if (apiStatus == 200)
                    {
