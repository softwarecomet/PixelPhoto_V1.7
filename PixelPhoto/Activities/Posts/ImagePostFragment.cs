...
{
    public class ImagePostFragment : Fragment
    {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            try
            {
                Mainview = inflater.Inflate(Resource.Layout.PixPhotoPost, container, false);
                ...
                if (addEvent)
                {
                    if (!string.IsNullOrEmpty(Json))
                    {
                        ...
                        if (!CommentCount.HasOnClickListeners)
                            CommentCount.Click += (sender, e) => ClickListeners.OnCommentPostClick(new CommentFeedClickEventArgs { View = Mainview, NewsFeedClass = item }, "ImagePost");

                        if (!LikeCount.HasOnClickListeners)
                            LikeCount.Click += (sender, e) => ClickListeners.OnLikedPostClick(new LikeNewsFeedClickEventArgs { View = Mainview, NewsFeedClass = item, LikeButton = LikeIcon });
                        ...
                        if (!UserAvatar.HasOnClickListeners)
                            UserAvatar.Click += (sender, e) => ClickListeners.OnAvatarImageFeedClick(new AvatarFeedClickEventArgs { NewsFeedClass = item, Image = UserAvatar, View = Mainview }, "ImagePost");

                        if (!CommentIcon.HasOnClickListeners)
                            CommentIcon.Click += (sender, e) => ClickListeners.OnCommentClick(new CommentFeedClickEventArgs { NewsFeedClass = item, View = Mainview }, "ImagePost");

                        if (!ViewCommentsButton.HasOnClickListeners)
                            ViewCommentsButton.Click += (sender, e) => ClickListeners.OnCommentClick(new CommentFeedClickEventArgs { NewsFeedClass = item, View = Mainview }, "ImagePost");

                        if (!MoreIcon.HasOnClickListeners)
                            MoreIcon.Click += (sender, e) => ClickListeners.OnMoreClick(new MoreFeedClickEventArgs { NewsFeedClass = item, View = Mainview, IsOwner = item.IsOwner }, false, "ImagePost");
        ...
        public void DisplayData(string fullname, string description, string avatar, string time, string videoFile, int likesCount, bool isLiked, bool isSaved, int votesCount)
        {
            try
            {
                ...
                LikeCount.Text = likesCount + " " + GetText(Resource.String.Lbl_Likes);
                LikeIcon.Tag = "Like";
                ...
                CommentCount.Text = votesCount + " " + GetText(Resource.String.Lbl_Comments);
