...
using Settings = PixelPhoto.AppSettings;
...
namespace PixelPhoto.Helpers.Utils
{
    ...
    public static partial class Methods
    {
        ...
        public static void DisplayReportResult(Activity activityContext, dynamic respond)
        {
            if (Settings.SetApisReportMode)
            {
                ...
                if (respond is ErrorObject errorMessage)
                {
                    ...
                    if (errorText.Contains("Invalid or expired access_token") || errorText.Contains("No session sent") || errorText.Contains("Please Login And Try Again"))
        ...
        public class AudioRecorderAndPlayer
        {
            ...
            public AudioRecorderAndPlayer()
            {
                try
                {
                    ...
                    SoundFileFullPath = new Java.IO.File(Path.AndroidDcimFolder + "/" + SoundFile);

                    var dir = Path.AndroidDcimFolder;
        ...
        public static class MultiMedia
        {
                ...
                bool saveOnPersonalFolder = true, bool saveOnGallaryFolder = false)
            {
                ...
                if (saveOnGallaryFolder)
                {
                    ...
                    if (System.IO.File.Exists(pathOfFile))
                    {
                        ...
                        MediaScannerConnection.ScanFile(Application.Context, new string[] { copyFileFullPath }, null, null);
            ...
            public static Bitmap Retrieve_VideoFrame_AsBitmap(Context context, string mediaFile, ThumbnailKind thumbnailKind = ThumbnailKind.MiniKind)
            {
                try
                {
                    ...
                    {
                        Bitmap bitmap = context.ContentResolver.LoadThumbnail(Uri.Parse(mediaFile), new Size(200, 200), null);
                        return bitmap;
                    }
                    else
                    {
                        Bitmap bitmap = ThumbnailUtils.CreateVideoThumbnail(new File(mediaFile), new Size(200, 200), new CancellationSignal());
                        return bitmap;
                    }
                }
                catch (Exception exception)
        ...
        public static class Time
        {
            ...
            public static string TimeAgo(int time, bool withReplace = false)
            {
                try
                {
            ...
            public static string TimeAgo(DateTime dateTime, bool withReplace = false)
            {
                try
                {
                    string result;
